...
float computeDerivative(float thet) {
  ...
  rawDtheta = (thet - lastAngle) / (dt / 1000000);
  //filteredDtheta = alpha * rawDtheta + (1.0 - alpha) * filteredDtheta;
  lastAngle = thet;
  return rawDtheta;
}

float computeIntegral(float thet) {
  dt = micros() - t_old_integral;
  t_old_integral = micros();
  rawItheta += thet * (dt / 1000000);
  return rawItheta;
}
...
float control(float theta) {
  ...
  if (-1 < theta && theta < 1) {
    integral = computeIntegral(theta);
    derivative = computeDerivate(theta);
    return kp * theta + ki * integral + kd * derivative ;
  }
  if (-0.1 < theta && theta < 0.1) {integral = 0;}

  integral = constrain(integral, -4, 4);
  // Serial.println(integral);

  // SWING UP IF ANGLE GREATER THAN -1/1
  return -_sign(computeDerivative(theta))*MAX_VOLTAGE*0.4;
