...
class Pendulum:
    ...
    def applyForces(self, wheel):
        ...
        # ── noise sources ────────────────────────────────────────────────────
        sensor_noise    = np.random.normal(0, 0)   # noisy angle measurement
        actuator_noise  = np.random.normal(0, 0)  # imperfect torque delivery
        disturbance     = np.random.normal(0, 0)  # random physical disturbance
...
class Reaction:
    ...
    def balanceTorque(self, pendulum):
        ...
        self.integral  = np.clip(self.integral,
                                 -self.maxtorque / max(self.ki, 1e-9),
                                  self.maxtorque / max(self.ki, 1e-9))
        torque = (self.kp * error + self.kd * pendulum.dtheta
    ...
    # ... mode switching logic unchanged ...
        ...
        # ── Saturation: if wheel is at maxspeed and torque pushes further, ──
        # ── it can't accelerate → no reaction torque delivered             ──
        ...
        pendulum.tm = torque
        return torque   # ← now reflects what the wheel *actually* delivered
