...
class Pendulum:
    ...
    #def applyForces(self, wheel):
        # Net gravity torque accounts for both bob and counterweight
    ...
    def applyForces(self, wheel):
        ...
        # Base acceleration disturbance (measured σ = 1.5635 m/s²)
        #sigma_base_acc = 1.5635
        #base_acc = np.random.normal(0, sigma_base_acc)
        #torque_disturbance = -(base_acc / self.l) * np.cos(self.theta) * self.I
        ...
        # ax disturbs the pendulum horizontally (like a push)
        # az disturbs it vertically (modulates effective gravity)
